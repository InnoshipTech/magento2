...
/**
 * Class CountryPudoFees
 * Dynamic rows for country-specific PUDO (locker) fees configuration
 * @package InnoShip\InnoShip\Block\Adminhtml\Form\Field
 */
class CountryPudoFees extends AbstractFieldArray
{
    /**
     * @var CountryColumn
     */
    ...
    public function getArrayRows()
    {
        ...
        if ($element->getValue() && is_array($element->getValue())) {
            foreach ($element->getValue() as $rowId => $row) {
                // Ensure row ID is prefixed with 'pudo_row_' (pudo = locker) to avoid numeric IDs and conflicts
                $rowId = is_numeric($rowId) ? 'pudo_row_' . $rowId : $rowId;
